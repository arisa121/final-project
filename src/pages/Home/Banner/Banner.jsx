...
const Banner = () => {
  return (
    <Swiper
      ...
      loop
      className="w-full h-[50vh] sm:h-[60vh] md:h-[70vh] lg:h-[75vh] rounded-b-2xl sm:rounded-b-3xl overflow-hidden"
    >
      {slides.map((slide) => (
        <SwiperSlide key={slide.id}>
          <div className="relative w-full h-full">
            ...
            {/* Dark Overlay */}
            <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/60 to-black/30 flex items-center">
              <motion.div
                initial={{ opacity: 0, y: 60 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ duration: 1 }}
                className="text-white w-full max-w-2xl px-4 sm:px-6 md:px-10 lg:px-16 space-y-3 sm:space-y-4 md:space-y-6"
              >
                ...
                <Link
                  to={slide.link}
                  className="btn btn-primary btn-sm sm:btn-md inline-flex items-center justify-center mt-2 sm:mt-4"
                >
                  {slide.button}
                </Link>
              </motion.div>
